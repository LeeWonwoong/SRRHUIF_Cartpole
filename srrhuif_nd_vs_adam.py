...
"""
=========================================================================
SRRHUIF-D3QN v4: Node Decoupled + SPAS + Adam Comparison
=========================================================================
Changes from v3:
  - Removed Full Vector and Layer Decoupled (ND only)
  - ★ Added Sigma Point Action Selection (SPAS) for h=0
  - ★ Added Adam D3QN for comparison
  - ★ Unified comparison plot (SRRHUIF ND vs Adam)
  - Mixed Precision preserved (FP32 forward / FP64 filter)
=========================================================================
"""
...
# =========================================================================
# 8. ★★★ SRRHUIF ND Step — with SPAS option ★★★
# =========================================================================
@torch.no_grad()
def srrhuif_step_nd(theta_current_in, theta_target, neuron_S_info, batch, sp,
    ...
    # =====================================================================
    # ★ DDQN Target — AFTER Phase 1a (unified buffer is now populated)
    # =====================================================================
    if is_first:
        if cfg.use_spas:
            # ★ Sigma Point Action Selection (SPAS)
            # unified buffer에 이미 모든 sigma points가 scatter됨
            # s_next에 한 번 더 forward하여 sigma point 평균으로 action 선택
            Q_sigma_f32 = forward_bmm(unified, info, s_next)        # (total_fwd, nA, batch_sz) FP32
            Q_sigma_mean = Q_sigma_f32.mean(dim=0)                  # (nA, batch_sz) — noise 상쇄
            a_best_next = Q_sigma_mean.argmax(dim=0)                # (batch_sz,)
            # Value evaluation: theta_target 단독
            ...
            # Standard DQN for h=0
            Q_tgt_f32 = forward_bmm(theta_target.squeeze().unsqueeze(0), info, s_next)
            q_val_next = Q_tgt_f32[0].max(dim=0).values.to(DTYPE)
    else:
        # h>0: DDQN with theta_current (previous horizon step result)
...
class LivePlotter:
    ...
    def __init__(self, method_name: str, max_episodes: int):
        ...
        # P_init plot (or epsilon for Adam)
            ...
            if buffer.current_size >= cfg.batch_size and steps_done % cfg.update_interval == 0:
                ...
                if len(batch_hist) == cfg.N_horizon:
                    for h in range(cfg.N_horizon):
                        is_first = (h == 0)
                        theta, neuron_S_info, l_val = srrhuif_step_nd(
...
def plot_comparison(srrhuif_log: LivePlotter, adam_log: LivePlotter):
    ...
    # Find first episode reaching 195
