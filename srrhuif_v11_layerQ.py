...
class Config:
    ...
    q_v: float = 1e-4    # V: Q가 작으므로 Y가 무한히 커질 수 있음 (빠른 닻 내리기)
    q_s1: float = 5e-4   # S1: 기존 수준의 중간 가소성 유지
    q_s0: float = 8e-3   # S0: 엄청난 Q 주입 -> Y_max가 절대 커지지 못함 (영구적 Plasticity)
    q_a: float = 5e-4    # A: 후반부 유연한 조향을 위해 넉넉한 Q 주입
    ...
    def __post_init__(self):
        ...
        self.param_str = f"a{self.alpha}_b{self.beta}_r{self.r_std}_q_v{self.q_v}_q_a{self.q_a}_q_s0{self.q_s0}_q_sq{self.q_s1}_p{self.p_init}_buffer{self.buffer_size}_batch{self.batch_size}_horizon{self.N_horizon}_tikhonov{self.tikhonov_lambda}"
...
parser.add_argument('--q_a', type=float, default=cfg.q_a)
parser.add_argument('--q_v', type=float, default=cfg.q_v)
parser.add_argument('--q_s0', type=float, default=cfg.q_s0)
parser.add_argument('--q_s1', type=float, default=cfg.q_s1)
...
# ★ [NEW] 레이어 타입에 따라 Q_std 값을 가져오는 헬퍼 함수
def get_layer_q_std(layer_type, local_idx, cfg):
    if 'value' in layer_type:
        return cfg.q_v
    elif 'advantage' in layer_type:
        return cfg.q_a
    elif 'shared' in layer_type:
        if local_idx == 0:
            return cfg.q_s0
        else:
            return cfg.q_s1
    return 5e-4  # fallback
...
class NDCache:
    def __init__(self, info: Dict, cfg: Config, device: str):
        ...
        for L, nd_layer in enumerate(info['nd_layers']):
            ...
            q_val = get_layer_q_std(nd_layer['type'], nd_layer['local_idx'], cfg)
            S_Q_cached = q_val * eye_n_per_batch.clone()
            ...
            self.layers[L] = {
                ...
                'q_std': q_val,  # ★ [NEW] 텐서 조립을 위해 q_val 저장
            }
        ...
        for n_per, grp in self.n_per_groups.items():
            ...
            offsets = [0]
            for L in grp['layers']:
                fo = info['nd_layers'][L]['fan_out']
                offsets.append(offsets[-1] + fo)
                layer_q = self.layers[L]['q_std']
                # 해당 레이어의 fan_out 만큼 Q 행렬 생성
                S_Q_layer = layer_q * torch.eye(n_per, dtype=DTYPE, device=device).unsqueeze(0).expand(fo, -1, -1)
                S_Q_list.append(S_Q_layer)
            
            grp['offsets'] = offsets
            grp['S_Q_grouped'] = torch.cat(S_Q_list, dim=0) # 하나로 결합
...
def train_srrhuif_nd():
    ...
    print(f"Horizon: {cfg.N_horizon}, Batch: {cfg.batch_size}, τ: {cfg.tau_srrhuif}")
    print(f"Layer Qs: [V={cfg.q_v}, S1={cfg.q_s1}, S0={cfg.q_s0}, A={cfg.q_a}]") # ★ Q값들 출력
...
def main():
    ...
    try:
        ...
        print(f"  Horizon: {cfg.N_horizon} | Batch: {cfg.batch_size}")
        print(f"  Layer Qs: V={cfg.q_v}, S1={cfg.q_s1}, S0={cfg.q_s0}, A={cfg.q_a}") # ★ Q값들 출력
