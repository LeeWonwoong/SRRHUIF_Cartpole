...
class Config:
    ...
    # === [TIER 2] Angle Diversity Injection ===
    # 초기 N 에피소드 동안 cart-pole 시작 state를 더 극단적으로 세팅
    # → Buffer에 다양한 pole angle 포함 → V1이 consistent direction 찾기 쉬움
    angle_inject_episodes: int = 0   # 몇 ep 동안 적용할지 (0이면 비활성)
    angle_inject_range: float = 0.15  # pole angle 초기값 범위 ±X rad (default 0.05)
    angvel_inject_range: float = 0.3  # pole angular velocity 초기값 범위 ±X (default ~0.05)
...
# [TIER 2] Angle diversity injection
parser.add_argument('--angle_inject_ep', type=int, default=cfg.angle_inject_episodes,
                    help='초기 N ep 동안 angle diversity 주입 (0=비활성)')
parser.add_argument('--angle_inject_range', type=float, default=cfg.angle_inject_range,
                    help='pole angle 초기화 범위 ±X rad (default 0.15)')
parser.add_argument('--angvel_inject_range', type=float, default=cfg.angvel_inject_range,
                    help='pole angular velocity 초기화 범위 ±X (default 0.3)')
...
# [TIER 2] angle injection
cfg.angle_inject_episodes = args.angle_inject_ep
cfg.angle_inject_range = args.angle_inject_range
cfg.angvel_inject_range = args.angvel_inject_range
...
def tria_operation_batch(A):
    ...
    d_clamped = torch.clamp(d_positive, min=1e-5)
...
def compute_full_cond_from_S(S_batch):
    ...
    try:
        ...
        y_max = eigvals_Y[:, -1].clamp(min=1e-8)  # 마지막이 max
        y_min = eigvals_Y[:, 0].clamp(min=1e-8)   # 첫 번째가 min
...
def _nd_time_update_core(theta_3d, P_sqrt_prev, S_Q_cached, eye_batch, gamma_val):
    ...
    # 1. 안전장치가 포함된 역행렬 계산으로 S_pred (정보행렬의 제곱근) 확보
    S_pred = safe_inv_tril_batch(P_sqrt_pred, eye_batch)
    
    # 2. [핵심 수정] Y를 만들지 않고 S_pred를 연쇄적으로 곱해 y_pred 계산
    # Y = S @ S^T 이므로, y = Y @ theta = S @ (S^T @ theta)
...
def _nd_compute_ht_core(Z_sigma_T_f32, Wm_col_f32, Wc_f32, zero_col_f32,
    ...
    P_xz_f64 = P_xz_f32.to(torch.float64)
    
    # [핵심 수정] Y를 곱하지 않고 S_pred를 연쇄적으로 곱해 HT 계산!
    # HT = Y @ Pxz = S @ (S^T @ Pxz)
    temp_ht = torch.bmm(S_pred_f64.transpose(-2, -1), P_xz_f64)
    HT_all = torch.bmm(S_pred_f64, temp_ht)
...
def _nd_meas_update_core(S_pred, y_pred, HT_all, theta_3d, residual_all, 
    ...
    # [TIER 1 #1,2] residual / innov 분포 (극값 & 분위수)
    residual_abs = torch.abs(residual_all)
    resid_max_val = torch.max(residual_abs).item()
    # q99 — batch 내 상위 1% 극단값 (outlier 포착)
    resid_flat = residual_abs.flatten()
    innov_flat = innov_abs.flatten()
    if resid_flat.numel() > 0:
        resid_q99_val = torch.quantile(resid_flat, 0.99).item()
        innov_q99_val = torch.quantile(innov_flat, 0.99).item()
    else:
        resid_q99_val = 0.0
        innov_q99_val = 0.0
    ...
    meas_stats = {
        ...
        'avg_P': avg_P_new,
        # [TIER 1] 새 필드
        'resid_max': resid_max_val,
        'resid_q99': resid_q99_val,
        'innov_q99': innov_q99_val,
    }
...
def srrhuif_step_nd(theta_current_in, theta_target, neuron_S_info, batch, sp,
    ...
    for n_per_val, grp in nd_cache.n_per_groups.items():
        ...
        S_pred_g, _, y_pred_g, X_sigma_g, scaled_P_g = _nd_time_update_core(
    ...
    # [TIER 1 #3] TD target 분포 분위수
    z_flat = z_measured.flatten()
    if z_flat.numel() > 0:
        z_q25 = torch.quantile(z_flat, 0.25).item()
        z_q75 = torch.quantile(z_flat, 0.75).item()
        z_q99 = torch.quantile(z_flat, 0.99).item()
        z_min = z_flat.min().item()
        z_max = z_flat.max().item()
        z_mean = z_flat.mean().item()
        z_std = z_flat.std().item() if z_flat.numel() > 1 else 0.0
    else:
        z_q25 = z_q75 = z_q99 = z_min = z_max = z_mean = z_std = 0.0

    # [TIER 1 #4] Batch state 통계 (pole angle, angular velocity)
    s_batch_cpu = s_batch if s_batch.dim() == 2 else s_batch.view(-1, s_batch.shape[-1])
    # CartPole state: [cart_pos, cart_vel, pole_angle, pole_ang_vel]
    angle_abs = s_batch_cpu[:, 2].abs()
    batch_angle_std = s_batch_cpu[:, 2].std().item() if s_batch_cpu.shape[0] > 1 else 0.0
    batch_angle_max = angle_abs.max().item()
    batch_angvel_std = s_batch_cpu[:, 3].std().item() if s_batch_cpu.shape[0] > 1 else 0.0
    terminal_ratio = batch['term'].float().mean().item()
    ...
    for n_per_val, grp in nd_cache.n_per_groups.items():
        ...
        # [TIER 1] 축적
        total_resid_max = max(total_resid_max, meas_stats['resid_max'])
        total_resid_q99 = max(total_resid_q99, meas_stats['resid_q99'])
        total_innov_q99 = max(total_innov_q99, meas_stats['innov_q99'])
    ...
    per_layer_ht = {}
    per_layer_delta = {}
    per_layer_delta_vec = {}  # [TIER 2] cosine similarity 계산용 delta vector
    ...
    for L in range(info['num_nd_layers']):
        ...
        w_diff = theta_new_flat[w_s:w_s+w_l] - theta_old_flat[w_s:w_s+w_l]
        b_diff = theta_new_flat[b_s:b_s+b_l] - theta_old_flat[b_s:b_s+b_l]
        w_delta = torch.norm(w_diff).item()
        b_delta = torch.norm(b_diff).item()
        per_layer_delta[label] = (w_delta**2 + b_delta**2)**0.5
        # [TIER 2] delta vector (CPU로 복사해 호출부에서 cosine 계산)
        per_layer_delta_vec[label] = torch.cat([w_diff.detach(), b_diff.detach()]).cpu()
    ...
    debug_stats = {
        ...
        'per_layer_delta_vec': per_layer_delta_vec,  # [TIER 2] V/A branch vector
        'per_layer_cond': per_layer_cond,
        'per_layer_ymax': per_layer_ymax,
        'per_layer_cond_full': per_layer_cond_full,
        # [TIER 1] 극값 (평균이 아닌 max)
        'resid_max': total_resid_max,
        'resid_q99': total_resid_q99,
        'innov_q99': total_innov_q99,
        # [TIER 1] TD target 분포
        'z_mean': z_mean,
        'z_std': z_std,
        'z_q25': z_q25,
        ...
        'z_min': z_min,
        'z_max': z_max,
        # [TIER 1] Batch state 통계
        'batch_angle_std': batch_angle_std,
        'batch_angle_max': batch_angle_max,
        'batch_angvel_std': batch_angvel_std,
...
def train_srrhuif_nd():
    ...
    # [TIER 2] Angle injection 설정 출력
    if cfg.angle_inject_episodes > 0:
        print(f"[TIER 2] Angle Injection: first {cfg.angle_inject_episodes} eps, "
              f"angle=±{cfg.angle_inject_range:.2f}rad ({np.degrees(cfg.angle_inject_range):.0f}°), "
              f"angvel=±{cfg.angvel_inject_range:.2f}")
    else:
        print(f"[TIER 2] Angle Injection: OFF")
    print(f"{'='*60}")
    ...
    for ep in range(1, cfg.max_episodes + 1):
        ...
        # ═══════════════════════════════════════════════════════════════════
        # [TIER 2] Angle Diversity Injection
        # 초기 N ep 동안 더 극단적 초기 state 강제 → buffer diversity 확보
        # CartPole env.unwrapped.state = [cart_x, cart_xdot, pole_ang, pole_angdot]
        # 기본 reset은 uniform(-0.05, 0.05)로 초기화 → diversity 부족
        # 주의: 네트워크 파라미터 `theta`와 이름 충돌 방지를 위해 pole_angle 사용
        # ═══════════════════════════════════════════════════════════════════
        inject_active = (ep <= cfg.angle_inject_episodes and cfg.angle_inject_episodes > 0)
        if inject_active:
            rng = np.random.RandomState(cfg.seed * 1000 + ep)
            cart_x = rng.uniform(-0.1, 0.1)
            cart_xdot = rng.uniform(-0.2, 0.2)
            pole_angle = rng.uniform(-cfg.angle_inject_range, cfg.angle_inject_range)
            pole_angdot = rng.uniform(-cfg.angvel_inject_range, cfg.angvel_inject_range)
            new_state = np.array([cart_x, cart_xdot, pole_angle, pole_angdot], dtype=np.float32)
        ...
        ep_q0, ep_q1 = [], []
        ep_i_mean, ep_i_max = [], []
        # [TIER 1] episode 내 update-level 극값/분위수 축적
        ep_resid_max, ep_resid_q99, ep_innov_q99 = [], [], []
        ep_z_std, ep_z_q99, ep_z_max = [], [], []
        ep_batch_angle_std, ep_terminal_ratio = [], []
        ep_delta_V1 = []  # 첫 update일 때 V1 폭주 추적
        # [TIER 2] Horizon 내부 V/A coherence
        ep_va_cos_h6, ep_va_cos_h7, ep_va_cos_h8 = [], [], []
        # [TIER 2] V0→S1 cascade 진단 (핵심: B vs A 결정적 차이 지표)
        ep_cascade_ratio = []  # V0/S1 Y_max / V1 Y_max (h=8) — cascade 얼마나 열렸는지
            ...
            if steps_done > cfg.warmup_step and buffer.current_size >= cfg.batch_size and steps_done % cfg.update_interval == 0:
                ...
                if len(batch_hist) == cfg.N_horizon:
                    ...
                    for h in range(cfg.N_horizon):
                        ...
                        # [TIER 1] update-level 극값 축적
                        ep_resid_max.append(dbg['resid_max'])
                        ep_resid_q99.append(dbg['resid_q99'])
                        ep_innov_q99.append(dbg['innov_q99'])
                        ep_z_std.append(dbg['z_std'])
                        ep_z_q99.append(dbg['z_q99'])
                        ep_z_max.append(dbg['z_max'])
                        ep_batch_angle_std.append(dbg['batch_angle_std'])
                        ep_terminal_ratio.append(dbg['terminal_ratio'])
                        # V1 delta (Tier1 #5: 초기 V1 폭주 감지)
                        v1_delta = dbg['per_layer_delta'].get('V1', 0.0)
                        ep_delta_V1.append(v1_delta)
                        
                        # ═══════════════════════════════════════════════════════
                        # [TIER 2] Horizon 후반 V0 vs A0 coherence
                        # 가설: 성공 = V0/A0 update direction이 h=6~8에서 coherent
                        #      붕괴 = V0/A0가 opposed (cos < 0) 방향으로 update
                        # 주의: V0/A0는 같은 shape (16→4 hidden, 68 params)
                        #       V1(5), A1(10)은 shape 달라 직접 비교 불가
                        #       → cascade 흐름상 같은 depth인 V0 vs A0만 사용
                        # ═══════════════════════════════════════════════════════
                        if h >= 6 and 'per_layer_delta_vec' in dbg:
                            dv = dbg['per_layer_delta_vec']
                            if 'V0' in dv and 'A0' in dv:
                                v_vec = dv['V0']
                                a_vec = dv['A0']
                                # 차원 체크 (양쪽 모두 68이어야 함)
                                if v_vec.numel() == a_vec.numel() and v_vec.numel() > 0:
                                    vn = torch.norm(v_vec).item()
                                    an = torch.norm(a_vec).item()
                                    if vn > 1e-10 and an > 1e-10:
                                        va_cos = (v_vec * a_vec).sum().item() / (vn * an)
                                    else:
                                        va_cos = 0.0
                                    if h == 6: ep_va_cos_h6.append(va_cos)
                                    elif h == 7: ep_va_cos_h7.append(va_cos)
                                    elif h == 8: ep_va_cos_h8.append(va_cos)
                        
                        # [TIER 2] Cascade ratio (V1만 터지고 V0/S1 못 따라가는지)
                        ymax_h = dbg['per_layer_ymax']
                        if 'V1' in ymax_h and 'V0' in ymax_h and 'S1' in ymax_h and ymax_h['V1'] > 0:
                            # V0, S1이 V1 대비 얼마나 성장했는지
                            cascade_r = (ymax_h['V0'] + ymax_h['S1']) / (2.0 * ymax_h['V1'])
                            ep_cascade_ratio.append(cascade_r)
        ...
        # [TIER 1 #6,7,8] Ref-Q 기반 붕괴 경고 지표
        min_q_margin = None
        ref_q_change_20ep = None
        sign_flips_20ep = None
        alert_level = 0
        if ref_q is not None:
            # #6. Min Q-margin: 5개 ref state 중 가장 작은 |ΔQ|
            q_margins = [abs(ref_q[n]['dq']) for n in REF_NAMES]
            min_q_margin = min(q_margins) if q_margins else 0.0
            
            # #7. Ref-Q 20ep 전과의 변화량 (logger가 기록 중인 history 이용)
            if len(logger.ref_dq_history['balance']) >= 20:
                change_vals = []
                for n in REF_NAMES:
                    hist = logger.ref_dq_history[n]
                    change_vals.append(abs(ref_q[n]['dq'] - hist[-20]))
                ref_q_change_20ep = float(np.mean(change_vals))
            
            # #8. Sign flip (fall_L의 부호 변화 빈도) in last 20 ep
            if len(logger.ref_dq_history['fall_L']) >= 20:
                recent = logger.ref_dq_history['fall_L'][-20:]
                signs = [1 if v > 0.01 else (-1 if v < -0.01 else 0) for v in recent]
                flips = sum(1 for i in range(1, len(signs)) 
                           if signs[i] != 0 and signs[i-1] != 0 and signs[i] != signs[i-1])
                sign_flips_20ep = flips
            
            # Alert level (0-4)
            if min_q_margin is not None and min_q_margin < 0.02: alert_level += 1
            if ref_q_change_20ep is not None and ref_q_change_20ep > 0.3: alert_level += 1
            if sign_flips_20ep is not None and sign_flips_20ep > 5: alert_level += 1
            if max_i_max > 5.0: alert_level += 1
        ...
        if ep % cfg.log_interval == 0:
            ...
            # ╔═══════════════════════════════════════════════════════════════╗
            # ║ [TIER 1] 정밀 디버깅 로그 — seed 실패/붕괴 규명               ║
            # ╚═══════════════════════════════════════════════════════════════╝
            # #1,2: Batch 내 residual/innov 극값 (평균이 아닌 max / q99)
            if ep_resid_max:
                resid_max_ep = max(ep_resid_max)
                resid_q99_ep = max(ep_resid_q99)
                innov_q99_ep = max(ep_innov_q99)
                print(f"          ├─▶ [T1] Extrema:      resid_max={resid_max_ep:.3f} "
                      f"resid_q99={resid_q99_ep:.3f} innov_q99={innov_q99_ep:.3f}")
            
            # #3: TD target 분포 (첫 update 또는 max)
            if ep_z_std:
                z_std_mean = np.mean(ep_z_std)
                z_max_max = max(ep_z_max)
                z_q99_max = max(ep_z_q99)
                print(f"          ├─▶ [T1] TD target:    z_std={z_std_mean:.3f} "
                      f"z_q99={z_q99_max:.2f} z_max={z_max_max:.2f}")
            
            # #4: Batch state manifold
            if ep_batch_angle_std:
                bangle_std_mean = np.mean(ep_batch_angle_std)
                terminal_mean = np.mean(ep_terminal_ratio)
                print(f"          ├─▶ [T1] Batch-state:  angle_std={bangle_std_mean:.4f} "
                      f"terminal_ratio={terminal_mean:.4f}")
            
            # #5: V1 early lock-in 감지 (Ep 1-20 특히 중요)
            if ep_delta_V1 and ep <= 30:
                v1_max_ep = max(ep_delta_V1) if ep_delta_V1 else 0
                v1_first = ep_delta_V1[0] if ep_delta_V1 else 0
                warn = "⚠️V1폭주" if v1_max_ep > 1.0 else ""
                print(f"          ├─▶ [T1] V1 Δθ:        first={v1_first:.4f} "
                      f"max={v1_max_ep:.4f} {warn}")
            
            # #6,7,8: 붕괴 경고 지표 (Ref Q 기반)
            if min_q_margin is not None:
                parts = [f"min_margin={min_q_margin:.4f}"]
                if ref_q_change_20ep is not None:
                    parts.append(f"Δref20ep={ref_q_change_20ep:.3f}")
                if sign_flips_20ep is not None:
                    parts.append(f"sign_flips20={sign_flips_20ep}")
                alert_str = f"ALERT={alert_level}/4"
                if alert_level >= 3:
                    alert_str = f"🔴{alert_str}"
                elif alert_level >= 2:
                    alert_str = f"🟡{alert_str}"
                print(f"          ├─▶ [T1] Collapse:     {' '.join(parts)} | {alert_str}")
            else:
                print(f"          ├─▶ [T1] Collapse:     (ref_q 미계산)")
            
            # ╔═══════════════════════════════════════════════════════════════╗
            # ║ [TIER 2] 구조적 진단 — V/A coherence + Cascade 완성도         ║
            # ╚═══════════════════════════════════════════════════════════════╝
            if ep_va_cos_h6 or ep_va_cos_h7 or ep_va_cos_h8:
                c6 = np.mean(ep_va_cos_h6) if ep_va_cos_h6 else 0
                c7 = np.mean(ep_va_cos_h7) if ep_va_cos_h7 else 0
                c8 = np.mean(ep_va_cos_h8) if ep_va_cos_h8 else 0
                # V0/A0 coherence: h=6~8 평균
                va_mean = np.mean([c for c in [c6, c7, c8] if c != 0]) if any([c6,c7,c8]) else 0
                warn = ""
                if va_mean < -0.3: warn = "⚠️V0/A0대립"
                elif va_mean > 0.3: warn = "✓V0/A0정렬"
                print(f"          ├─▶ [T2] V0/A0 cos:    h6={c6:+.3f} h7={c7:+.3f} h8={c8:+.3f} "
                      f"(mean={va_mean:+.3f}) {warn}")
            
            if ep_cascade_ratio:
                cascade_mean = np.mean(ep_cascade_ratio)
                cascade_min = min(ep_cascade_ratio)
                # Cascade 성숙도 해석 기준
                # < 0.01: V1만 성장, 하위 레이어 침묵 (실패/진동 위험)
                # 0.01 ~ 1.0: cascade 성장 중
                # > 1.0: V0/S1이 V1보다 큼 (건강한 완성)
                if cascade_mean < 0.01: c_warn = "⚠️V1독주"
                elif cascade_mean < 0.1: c_warn = "🟡cascade_중"
                else: c_warn = "✓cascade_성숙"
                print(f"          └─▶ [T2] Cascade:      (V0+S1)/2V1: mean={cascade_mean:.3f} min={cascade_min:.3f} {c_warn}")
