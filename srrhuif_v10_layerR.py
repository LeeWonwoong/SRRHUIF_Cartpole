...
class Config:
    ...
    def __post_init__(self):
        ...
        self.param_str = f"a{self.alpha}_b{self.beta}_rv{self.r_v}_rs1{self.r_s1}_rs0{self.r_s0}_ra{self.r_a}_p{self.p_init}_q{self.q_std}_buffer{self.buffer_size}_batch{self.batch_size}_horizon{self.N_horizon}_tikhonov{self.tikhonov_lambda}"
...
parser.add_argument('--r_v', type=float, default=cfg.r_v)
parser.add_argument('--r_s1', type=float, default=cfg.r_s1)
parser.add_argument('--r_s0', type=float, default=cfg.r_s0)
parser.add_argument('--r_a', type=float, default=cfg.r_a)
...
# ★ 레이어 타입과 인덱스를 보고 해당하는 R 값을 반환하는 헬퍼 함수
def get_layer_r_std(layer_type, local_idx, cfg):
...
class NDCache:
    def __init__(self, info: Dict, cfg: Config, device: str):
        ...
        for L, nd_layer in enumerate(info['nd_layers']):
            ...
            # 이 레이어만의 R 값 할당
            r_val = get_layer_r_std(nd_layer['type'], nd_layer['local_idx'], cfg)
            r_inv_sqrt_val = 1.0 / r_val
            r_inv_val = 1.0 / (r_val ** 2)
            ...
            self.layers[L] = {
                ...
                'r_inv_sqrt': r_inv_sqrt_val, 'r_inv': r_inv_val,
            }
        ...
        for n_per, grp in self.n_per_groups.items():
            ...
            #  그룹 내 뉴런들의 R 값을 하나의 거대한 (total_n, 1, 1) 텐서로 조립
            r_inv_sqrt_list = []
            r_inv_list = []
            ...
            for L in grp['layers']:
                ...
                r_inv_sqrt_list.append(torch.full((fo, 1, 1), self.layers[L]['r_inv_sqrt'], dtype=DTYPE, device=device))
                r_inv_list.append(torch.full((fo, 1, 1), self.layers[L]['r_inv'], dtype=DTYPE, device=device))
            
            grp['offsets'] = offsets
            grp['r_inv_sqrt_grouped'] = torch.cat(r_inv_sqrt_list, dim=0)
            grp['r_inv_grouped'] = torch.cat(r_inv_list, dim=0)
...
def srrhuif_step_nd(theta_current_in, theta_target, neuron_S_info, batch, sp,
    ...
    for n_per_val, grp in nd_cache.n_per_groups.items():
        ...
        theta_new_g, S_new_g, meas_stats = _nd_meas_update_core(
            ...
            all_theta_3d,
            all_residual, 
            grp['r_inv_sqrt_grouped'],  # ← cfg.r_inv_sqrt 대신 교체
            grp['r_inv_grouped'],       # ← cfg.r_inv 대신 교체
            grp['eye_grouped'],
...
def train_srrhuif_nd():
    ...
    print(f"Settings: [α={cfg.alpha}, β={cfg.beta}, P={cfg.p_init}]")
