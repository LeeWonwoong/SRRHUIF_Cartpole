...
"""
=========================================================================
SRRHUIF-D3QN v4.2: Adaptive P + Target Variance(T_Var) Debugging + FP32
=========================================================================
Changes from v4.1:
  - ★ Changed to Pure FP32 representation (torch.float32)
  - ★ Adjusted JITTER from 1e-14 to 1e-8 for FP32 numerical stability
  - ★ Removed all hardcoded .double() and .to(torch.float64) conversions
=========================================================================
"""

print("=" * 70)
print(f"SRRHUIF-D3QN v4.2 (Pure FP32 Debug) | PyTorch: {torch.__version__}")
...
# ★ 전체 기본 dytpe을 FP32로 변경
torch.set_default_dtype(torch.float32)
DTYPE = torch.float32
DTYPE_FWD = torch.float32
# ★ FP32의 수치적 한계를 고려해 JITTER 값을 상향 조정
JITTER = 1e-8
...
def _nd_compute_ht_core(Z_sigma_T_f32, Wm_col_f32, Wc_f32, zero_col_f32,
    ...
    residual_all = z_measured_exp - z_hat_f32
    HT_all = torch.bmm(Y_pred, P_xz_f32)
    return HT_all, residual_all, z_hat_f32
...
def srrhuif_step_nd(theta_current_in, theta_target, neuron_S_info, batch, sp,
    ...
    # =====================================================================
    # Phase 1a: Time Update (FP32) + Scatter to FP32 unified buffer
    # =====================================================================
    ...
    # =====================================================================
    # Phase 1c: HT Computation — FP32 Core
    # =====================================================================
    layer_data = []
    for L in range(info['num_nd_layers']):
        td = time_data[L]
        ...
        HT_all, residual_all, z_hat_f32 = _nd_compute_ht_core(
    ...
    # =====================================================================
    # Phase 2: Measurement Update (FP32)
    # =====================================================================
...
def train_adam():
    ...
    device = cfg.device
    # ★ .double() 제거 (FP32 기본 모델로 사용)
    policy_net = D3QNModule(dimS, nA, cfg).to(device)
    target_net = D3QNModule(dimS, nA, cfg).to(device)
...
def plot_comparison(srrhuif_log: LivePlotter, adam_log: LivePlotter):
    ...
    plt.tight_layout()
    filename = f"compare_SRRHUIF-ND_{spas_str}_vs_Adam_R{cfg.r_std}_Q{cfg.q_std}_g{cfg.gamma}_FP32.png"
...
def main():
    print(f"\n{'#' * 70}")
    print(f"  SRRHUIF-ND ({'SPAS' if cfg.use_spas else 'Standard'}) vs Adam D3QN Comparison (FP32)")
