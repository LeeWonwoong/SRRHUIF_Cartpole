...
def tria_operation_batch(A):
    ...
    d_clamped = torch.clamp(d_positive, min=1e-8)
...
def _nd_compute_ht_core(Z_sigma_T_f32, Wm_col_f32, Wc_f32, zero_col_f32,
    ...
    # [추가할 부분] Batch L2 Norm Clipping
    max_norm = 100.0  # 임계치 설정 (관찰된 정상 h=0, 1 스케일 기준)
    
    # 1. 각 노드별 자코비안 벡터의 L2 Norm 계산 (dim=1 유지)
    norms = torch.norm(HT_all, p=2, dim=1, keepdim=True)
    
    # 2. 클리핑 계수 계산 (Norm이 max_norm보다 크면 축소 비율 계산, 작으면 1.0)
    clip_coef = max_norm / (norms + 1e-8)
    clip_coef = torch.clamp(clip_coef, max=1.0)
    
    # 3. 방향(비율)은 유지한 채 스케일만 안전하게 축소
    HT_all = HT_all * clip_coef
...
def initialize_theta(info, device):
    ...
    for layer in info['layers']:
        ...
        last_v_layer_idx = info['value_end_idx']-1
        last_v_layer = info['layers'][last_v_layer_idx]

        b_start = last_v_layer['b_start']
        b_len = last_v_layer['b_len']
        theta[b_start : b_start + b_len] = 10.0
    return theta
