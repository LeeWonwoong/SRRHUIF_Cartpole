...
"""
=========================================================================
SRRHUIF-D3QN v6.0: Optimized FIR (Grouped QR + Target Q Cache)
=========================================================================
Changes from v5.0:
  1. n_per grouped batched QR: 6 QR calls → 2 per phase (time/meas update)
  2. Target Q caching: Q_target(s_next) pre-computed before horizon loop
  3. Adaptive P removed: fixed p_init, no external schedule
  4. Config: horizon=16, batch=32, q_std=8e-3, tau=0.01
=========================================================================
"""
...
def forward_bmm_cached(unified, info, x, theta_f32, nd_cache):
    """
    Activation-cached version of forward_bmm for ND sigma points.
    ND의 핵심: Layer L의 sigma point는 Layer L 파라미터만 변경.
    → Layer L 이전의 activation을 base theta로 미리 계산(cache)하고,
      각 ND layer의 sigma point는 해당 layer부터만 forward.
    
    Returns: same shape as forward_bmm(unified, info, x) → (total_fwd, nA, batch_sz)
    """
    x = x.to(DTYPE_FWD)
    if x.dim() == 1: x = x.unsqueeze(1)
    if x.shape[0] != info['dimS']: x = x.t()
    ...
    # ================================================================
    # Step 1: Base forward pass — cache activation at each layer input
    # ================================================================
    h = x
    shared_pre = [x]  # shared_pre[i] = input to shared layer i
    for i in range(info['shared_end_idx']):
        ly = info['layers'][i]
        W = theta_f32[ly['W_start']:ly['W_start'] + ly['W_len']].view(ly['W_shape'])
        b = theta_f32[ly['b_start']:ly['b_start'] + ly['b_len']].view(-1, 1)
        h = F.relu(W @ h + b)
        shared_pre.append(h)
    shared_out = h
    ...
    # Value head base
    v = shared_out
    val_pre = [shared_out]  # val_pre[i] = input to value layer i
    for i in range(info['shared_end_idx'], info['value_end_idx']):
        ly = info['layers'][i]
        W = theta_f32[ly['W_start']:ly['W_start'] + ly['W_len']].view(ly['W_shape'])
        b = theta_f32[ly['b_start']:ly['b_start'] + ly['b_len']].view(-1, 1)
    ...
    # Advantage head base
    a = shared_out
    adv_pre = [shared_out]  # adv_pre[i] = input to advantage layer i
    for i in range(info['value_end_idx'], len(info['layers'])):
        ly = info['layers'][i]
        W = theta_f32[ly['W_start']:ly['W_start'] + ly['W_len']].view(ly['W_shape'])
        b = theta_f32[ly['b_start']:ly['b_start'] + ly['b_len']].view(-1, 1)
        ...
        a = F.relu(z) if i < len(info['layers']) - 1 else z
        adv_pre.append(a)
    base_a = a  # (nA, batch_sz)
    
    # ================================================================
    # Step 2: Per-ND-layer — forward from cached activation only
    # ================================================================
    total_fwd = nd_cache.total_forwards
    Q_all = torch.empty(total_fwd, nA, batch_sz, dtype=DTYPE_FWD, device=device)
    
    for L in range(info['num_nd_layers']):
        nd = info['nd_layers'][L]
        fwd_s, fwd_e = nd_cache.layer_fwd_slices[L]
        sth = unified[fwd_s:fwd_e]  # (n_sigma_L, total_params)
        ns = sth.shape[0]
        ltype, lidx = nd['type'], nd['local_idx']
        
        if ltype == 'shared':
            # Start from cached input to this shared layer
            cached = shared_pre[lidx]  # (dim, batch_sz)
            h = cached.unsqueeze(0).expand(ns, -1, -1)
            # Forward through remaining shared layers
            for i in range(lidx, info['shared_end_idx']):
                ly = info['layers'][i]
                od, id_ = ly['W_shape']
                W = sth[:, ly['W_start']:ly['W_start'] + ly['W_len']].view(ns, od, id_)
                b = sth[:, ly['b_start']:ly['b_start'] + ly['b_len']].view(ns, od, 1)
                h = F.relu(torch.bmm(W, h) + b)
            so = h
            # Value head (base params in sth, since ND only perturbs current layer)
            v = so
            for i in range(info['shared_end_idx'], info['value_end_idx']):
                ly = info['layers'][i]
                od, id_ = ly['W_shape']
            ...
            for i in range(info['value_end_idx'], len(info['layers'])):
                ...
                z = torch.bmm(W, a) + b
                a = F.relu(z) if i < len(info['layers']) - 1 else z
            Q_all[fwd_s:fwd_e] = v + (a - a.mean(dim=1, keepdim=True))
        
        elif ltype == 'value':
            # Only value head changes; advantage = base
            cached = val_pre[lidx]
            v = cached.unsqueeze(0).expand(ns, -1, -1)
            start_i = info['shared_end_idx'] + lidx
            for i in range(start_i, info['value_end_idx']):
                ly = info['layers'][i]
                od, id_ = ly['W_shape']
                W = sth[:, ly['W_start']:ly['W_start'] + ly['W_len']].view(ns, od, id_)
                b = sth[:, ly['b_start']:ly['b_start'] + ly['b_len']].view(ns, od, 1)
                z = torch.bmm(W, v) + b
                v = F.relu(z) if i < info['value_end_idx'] - 1 else z
            # Advantage is base (no perturbation)
            a_exp = base_a.unsqueeze(0).expand(ns, -1, -1)
            Q_all[fwd_s:fwd_e] = v + (a_exp - a_exp.mean(dim=1, keepdim=True))
        
        elif ltype == 'advantage':
            # Only advantage head changes; value = base
            cached = adv_pre[lidx]
            a = cached.unsqueeze(0).expand(ns, -1, -1)
            start_i = info['value_end_idx'] + lidx
            for i in range(start_i, len(info['layers'])):
                ly = info['layers'][i]
                od, id_ = ly['W_shape']
                W = sth[:, ly['W_start']:ly['W_start'] + ly['W_len']].view(ns, od, id_)
                b = sth[:, ly['b_start']:ly['b_start'] + ly['b_len']].view(ns, od, 1)
                z = torch.bmm(W, a) + b
                a = F.relu(z) if i < len(info['layers']) - 1 else z
            # Value is base (no perturbation)
            v_exp = base_v.unsqueeze(0).expand(ns, -1, -1)
            Q_all[fwd_s:fwd_e] = v_exp + (a - a.mean(dim=1, keepdim=True))
...
def srrhuif_step_nd(theta_current_in, theta_target, neuron_S_info, batch, sp,
    ...
    if q_next_target_cached is not None:
        # ★ Target Q는 캐시에서 가져옴 — forward_bmm(theta_target) 제거
        if is_first and cfg.use_spas:
            # SPAS: sigma point 평균으로 action selection (이건 여전히 필요)
            Q_sigma_f32 = forward_bmm(unified, info, s_next)
            a_best_next = Q_sigma_f32.mean(dim=0).argmax(dim=0)
        else:
            # D3QN: theta_current로 action selection만
    ...
    # =====================================================================
    # Phase 3: H computation — Activation Cached forward
    # =====================================================================
    Q_all_f32 = forward_bmm_cached(unified, info, s_batch, theta_current.squeeze().to(DTYPE_FWD), nd_cache)
    ...
    for n_per_val, grp in nd_cache.n_per_groups.items():
        ...
        # Per-group HT/residual norm (averaged across layers in group)
    ...
    # ★ Per-layer breakdown: H^T norm & parameter delta norm
    per_layer_ht = {}
    per_layer_delta = {}
    ...
    for L in range(info['num_nd_layers']):
        ...
        ltype = nd['type']
        lidx = nd['local_idx']
        label = f"{ltype[0].upper()}{lidx}"  # S0, S1, V0, V1, A0, A1
        
        per_layer_ht[label] = per_layer[L]['ht_norm']
        
        # per-layer delta: W and b parameter change
    ...
    debug_stats = {
        ...
        # ★ NEW: per-layer breakdown
        'per_layer_ht': per_layer_ht,       # {S0: 1.2, S1: 300, V0: 5, ...}
        'per_layer_delta': per_layer_delta,  # {S0: 0.05, S1: 0.12, ...}
    }
...
def train_srrhuif_nd():
    ...
    print(f"Optimizations: Grouped QR + Target Q Cache")
    print(f"{'='*60}")
    ...
    # ★ Activation Caching 검증
    with torch.no_grad():
        # [수정] 차원 순서를 (Batch, dimS) 형태로 맞춰줍니다.
        test_x = torch.randn(32, dimS, dtype=DTYPE_FWD, device=cfg.device)
        unified_test = nd_cache.unified_thetas
        unified_test[:] = theta.squeeze().to(DTYPE_FWD)
        # 기존 sigma point scatter (임의 perturbation)
        for L in range(info['num_nd_layers']):
            lc = nd_cache.get(L)
            nd_layer = info['nd_layers'][L]
            fan_in, fan_out, n_per = lc['fan_in'], lc['fan_out'], lc['n_per']
            fwd_s, fwd_e = nd_cache.layer_fwd_slices[L]
            # perturb sigma points slightly
            perturb = 0.01 * torch.randn(fan_out, lc['num_sigma'], fan_in, dtype=DTYPE_FWD, device=cfg.device)
            layer_view = unified_test[fwd_s:fwd_e].view(fan_out, lc['num_sigma'], -1)
            layer_view.scatter_add_(dim=2, index=lc['w_col_idx'], src=perturb)
        
        Q_orig = forward_bmm(unified_test, info, test_x)
        Q_cached = forward_bmm_cached(unified_test, info, test_x, theta.squeeze().to(DTYPE_FWD), nd_cache)
        max_diff = (Q_orig - Q_cached).abs().max().item()
        print(f"  [✓] Activation Cache 검증: max diff = {max_diff:.2e} {'(PASS)' if max_diff < 1e-5 else '(FAIL!)'}")
    ...
    for ep in range(1, cfg.max_episodes + 1):
        ...
        last_h_layer_delta = []
        last_ep_cos = None
        last_horizon_cos = None
        ep_horizon_cos_list = []   # 에피소드 내 모든 호라이즌 간 cos 기록
        prev_horizon_net_delta = None  # 이전 호라이즌의 전체 이동 벡터
        ...
        for t in range(cfg.max_steps):
            ...
            if buffer.current_size >= cfg.batch_size and steps_done % cfg.update_interval == 0:
                ...
                if len(batch_hist) == cfg.N_horizon:
                    ...
                    # ★ 호라이즌 전체 net delta 측정용
                    theta_before_horizon = theta.squeeze().clone()
                    ...
                    # ★ 호라이즌 전체 net delta → 이전 호라이즌과 cosine 비교
                    horizon_net_delta = theta.squeeze() - theta_before_horizon
                    horizon_net_norm = torch.norm(horizon_net_delta).item()
                    if prev_horizon_net_delta is not None and horizon_net_norm > 1e-10 and torch.norm(prev_horizon_net_delta) > 1e-10:
                        hcos = F.cosine_similarity(horizon_net_delta.unsqueeze(0), prev_horizon_net_delta.unsqueeze(0)).item()
                        ep_horizon_cos_list.append(hcos)
                        last_horizon_cos = hcos
                    prev_horizon_net_delta = horizon_net_delta.clone()
        ...
        if ep % 5 == 0:
            ...
            if last_h_k_traj:
                ...
                # ★ Horizon-level direction consistency
                hcos_str = f"{last_horizon_cos:+.3f}" if last_horizon_cos is not None else "N/A"
                avg_hcos = np.mean(ep_horizon_cos_list) if ep_horizon_cos_list else 0.0
                n_hcalls = len(ep_horizon_cos_list) + 1
                print(f"          └─▶ horizon_cos: {hcos_str} (avg: {avg_hcos:+.3f}, n={n_hcalls})   ← 연속 호라이즌 간 방향 일관성")
                # ★ Episode-level
