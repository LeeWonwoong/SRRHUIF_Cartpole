...
def srrhuif_step_nd(theta_current_in, theta_target, neuron_S_info, batch, sp,
    ...
    debug_stats = {
        ...
        # ★ innov 분해: innov = (z-ẑ) + H^T·θ
        'resid_in_innov': total_resid_in_innov / gc,       # |z - ẑ| 기여
        'ht_theta_in_innov': total_ht_theta_in_innov / gc,  # |H^T θ| 기여  
        'innov_norm': total_innov_norm / gc,                 # |innov| 전체
    }
    ...
    for ep in range(1, cfg.max_episodes + 1):
        ...
        if ep % 5 == 0:
            ...
            if last_h_k_traj:
                ...
                print(f"          └─▶ ||H^T||/h: {fmt(last_h_ht_traj)}")
                # innov = (z-ẑ) + H^T·θ  분해
                if last_h_innov_decomp:
                    r_parts = [d[0] for d in last_h_innov_decomp]  # |z-ẑ|
                    h_parts = [d[1] for d in last_h_innov_decomp]  # |H^T·θ|
                    i_parts = [d[2] for d in last_h_innov_decomp]  # |innov|
                    print(f"          └─▶ |z-ẑ|/h:   {fmt(r_parts)}   ← residual in innov")
                    print(f"          └─▶ |H^Tθ|/h:  {fmt(h_parts)}   ← prior info term")
                    print(f"          └─▶ |innov|/h:  {fmt(i_parts)}   ← total")
