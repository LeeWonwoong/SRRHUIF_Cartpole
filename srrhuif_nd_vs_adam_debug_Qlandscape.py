...
"""
=========================================================================
SRRHUIF-D3QN v4.2: Automated Logging & 4-Plot Landscape Analysis
=========================================================================
"""

print("=" * 70)
print(f"SRRHUIF-D3QN v4.2 (Automated Logging) | PyTorch: {torch.__version__}")
...
class Config:
    ...
    N_horizon: int = 5
    q_std: float = 3e-3
    r_std: float = 1.8  # ★ 여기서 파라미터를 바꿔가며 테스트하세요!
    ...
    def __post_init__(self):
        ...
        # ★ 수정된 부분: results_cartpole 폴더 안에 '파라미터 이름'으로 된 하위 폴더를 만듭니다!
...
parser.add_argument('--r_std', type=float, default=1.8)
parser.add_argument('--p_max', type=float, default=0.013)
...
# =========================================================================
# ★ 10. Live Plotter (4-Subplots & File Naming)
# =========================================================================
class LivePlotter:
    def __init__(self, method_name: str, max_episodes: int, param_str: str = ""):
        ...
        # ★ 1x4 Subplots for Z_var inclusion
        ...
        if 'SRRHUIF' in method_name: self.ax_p.set_ylim(0, cfg.p_init_max * 1.2)
    ...
    def add(self, reward, loss, p_init=0.0, z_var=0.0):
    ...
    def refresh(self):
        ...
        self.line_z.set_data(ep_range, self.z_vars) # Z_var 업데이트
...
# =========================================================================
# ★ 11. Landscape Visualization Functions (Naming Updated)
# =========================================================================
...
def train_srrhuif_nd():
    ...
    for ep in range(1, cfg.max_episodes + 1):
            ...
            if buffer.current_size >= cfg.batch_size and steps_done % cfg.update_interval == 0:
                ...
                if len(batch_hist) == cfg.N_horizon:
                    for h in range(cfg.N_horizon):
                        ...
                        ep_l.append(l_val); ep_var.append(t_var)
        ...
        # ★ 로거에 z_var 함께 저장
        logger.add(ep_r, avg_l, p_init, avg_v)
        ...
        if ep % 5 == 0:
            ...
            print(f"[SRRHUIF] Ep {ep:3d} | Rwd: {ep_r:6.1f} | Avg20: {recent:6.1f} "
                  f"| Loss: {avg_l:.4f} | Z_var: {avg_v:.2f} | P_init: {p_init:.4f} | Time: {time.time()-ep_start:.2f}s")
...
def train_adam():
    ...
    logger = LivePlotter("Adam D3QN", cfg.max_episodes, cfg.param_str)
    steps_done = 0
    train_start_time = time.time()
    update_times = []
    ...
    for ep in range(1, cfg.max_episodes + 1):
        ...
        for t in range(cfg.max_steps):
            ...
            if buffer.current_size >= cfg.batch_size and steps_done % cfg.update_interval == 0:
                update_start = time.perf_counter()
                batch = buffer.sample_batch(cfg.batch_size)
                s_b, s_next_b = batch['s'].t(), batch['s_next'].t()
                if normalizer:
                    s_b, s_next_b = normalizer.normalize(s_b), normalizer.normalize(s_next_b)
        ...
        # Adam은 P_init과 Z_var가 없으므로 0 전달
        logger.add(ep_r, avg_l, 0.0, 0.0)
...
def plot_comparison(srrhuif_log: LivePlotter, adam_log: LivePlotter):
    ...
    summary = (
        ...
        f"{'Max reward':<25} {max(srrhuif_log.rewards):>10.1f} {max(adam_log.rewards):>10.1f}\n"
    ...
    plt.tight_layout()
    filename = os.path.join(cfg.outdir, f"{cfg.param_str}_Compare_vs_Adam.png")
    plt.savefig(filename, dpi=150)
    print(f"\n[*] 비교 그래프 저장 완료: {filename}")
    plt.close()
