...
"""
=========================================================================
SRRHUIF-D3QN v5.0: Advanced Debugging (1x5 Plot + 2D Trajectory Map)
=========================================================================
"""

print("=" * 70)
print(f"SRRHUIF-D3QN v5.0 (Advanced Debugging) | PyTorch: {torch.__version__}")
...
class Config:
    ...
    # Adaptive P_init variables
    p_init_min: float = 0.001
    p_init_max: float = 0.018
    ...
    def __post_init__(self):
        ...
        self.param_str = f"a{self.alpha}_b{self.beta}_r{self.r_std}_p{self.p_init_max}"
...
parser.add_argument('--p_max', type=float, default=cfg.p_init_max)
...
def srrhuif_step_nd(theta_current_in, theta_target, neuron_S_info, batch, sp, is_first, p_init_val, nd_cache):
    ...
    for L in range(info['num_nd_layers']):
        td, lc, fan_out = time_data[L], time_data[L]['lc'], time_data[L]['fan_out']
        fwd_start, fwd_end = nd_cache.layer_fwd_slices[L]
        ...
        layer_data.append({
            'nd_layer': td['nd_layer'], 'lc': lc, 'fan_in': td['fan_in'], 'fan_out': fan_out,
            'W_start': td['W_start'], 'b_start': td['b_start'], 'theta_all_prior': td['theta_all_prior'],
            'theta_all_prior_3d': td['theta_all_prior_3d'], 'S_pred': td['S_pred'], 'y_pred': td['y_pred'],
            'HT_all': HT_all, 'residual_all': residual_all, 'loss': torch.mean(residual_all ** 2)
        })
        layer_count += 1

    for L, data in enumerate(layer_data):
        theta_new_all, S_new_all = _nd_meas_update_core(
            data['S_pred'], data['y_pred'], data['HT_all'], data['theta_all_prior_3d'], 
            data['residual_all'], cfg.r_inv_sqrt, cfg.r_inv, data['lc']['eye_n_per_batch'])
    ...
    # ★ 가중치 변화량 L2 Norm 계산 (Kalman Gain Proxy)
    k_gain_norm = torch.norm(theta_current_in.squeeze() - theta_current.squeeze()).item()

    return theta_current, new_S_info, (total_loss / layer_count).item(), target_var, k_gain_norm

# =========================================================================
# ★ 10. Live Plotter (5-Subplots & File Naming)
# =========================================================================
class LivePlotter:
    def __init__(self, method_name: str, max_episodes: int, param_str: str = ""):
        ...
        self.k_gains = [] # ★ 칼만 게인 저장 리스트 추가
        self.total_time, self.avg_step_time = 0.0, 0.0
        
        # ★ 1x5 Subplots for Kalman Gain inclusion
        ...
        # ★ Weight Update Norm (Kalman Gain Proxy)
    ...
    def add(self, reward, loss, p_init=0.0, z_var=0.0, k_gain=0.0): # ★ k_gain 파라미터 추가
        self.rewards.append(reward)
        self.losses.append(max(loss, 1e-10))
        self.p_inits.append(p_init)
        self.z_vars.append(z_var)
        self.k_gains.append(k_gain) # ★ 리스트 추가
    ...
    def refresh(self):
        ...
        self.line_k.set_data(ep_range, self.k_gains) # ★ 플롯 데이터 세팅
...
def plot_srrhuif_loss_landscape(theta_star, info, buffer, cfg, method_name, param_str, resolution=21, span=0.5):
    ...
    torch.manual_seed(42) # 시드 고정 (Trajectory 지도와 축을 맞추기 위해)
    ...
    spread = cfg.alpha * np.sqrt(cfg.p_init_max)
...
def plot_srrhuif_2d_trajectory(theta_star, theta_history, p_inits, info, buffer, cfg, method_name, param_str, resolution=30, span=0.6):
    ...
    torch.manual_seed(42) # 3D 지형도와 동일한 기준축 설정
    d1 = torch.randn(n_params, dtype=DTYPE, device=device)
    d2 = torch.randn(n_params, dtype=DTYPE, device=device)
    d2 = d2 - (torch.dot(d1, d2) / torch.dot(d1, d1)) * d1 
    d1 = d1 / torch.norm(d1); d2 = d2 / torch.norm(d2)
    ...
    if len(traj_x) >= 80:
        ...
        ax.scatter(traj_x[ep80_idx], traj_y[ep80_idx], color='blue', s=80, label='Shockwave Hit (Ep 80)')
...
def plot_q_landscape_timelapse(theta_snapshots, info, cfg, normalizer, method_name, param_str, resolution=40):
    ...
    # 2행 3열 등 스냅샷 개수에 맞게 그리드 설정 (최대 6개)
    ...
    global_vmin, global_vmax = 0, 20 # 일반적인 CartPole Q값 범위
...
def plot_1d_loss_evolution(theta_snapshots, info, buffer, cfg, method_name, param_str, resolution=50, span=0.5):
    ...
    ax.set_yscale('log') # 폭발적인 Loss를 보기 위해 Log 스케일 적용
...
def train_srrhuif_nd():
    ...
    print(f"Settings: [R={cfg.r_std}, α={cfg.alpha}, β={cfg.beta}, P_max={cfg.p_init_max}]")
    print(f"{'='*60}")
    ...
    for ep in range(1, cfg.max_episodes + 1):
            ...
            if buffer.current_size >= cfg.batch_size and steps_done % cfg.update_interval == 0:
                ...
                if len(batch_hist) == cfg.N_horizon:
                    for h in range(cfg.N_horizon):
                        is_first = (h == 0)
                        # ★ 5번째 리턴값 t_k_gain 받기
                        theta, neuron_S_info, l_val, t_var, t_k_gain = srrhuif_step_nd(
                            theta, theta_target, neuron_S_info, batch_hist[h], sp, is_first, p_init, nd_cache)
        ...
        avg_k = np.mean(ep_k_gain) if ep_k_gain else 0 # ★ 에피소드 평균 칼만 게인
        
        # ★ 로거에 k_gain 함께 저장
        logger.add(ep_r, avg_l, p_init, avg_v, avg_k)
        
        # ★ 에피소드 종료 후 현재 가중치 저장
        theta_history.append(theta.squeeze().clone().cpu().numpy())
        
        # ★ 추가: 20 에피소드마다(또는 마지막) 타임랩스용 스냅샷 저장
        ...
        if ep % 5 == 0:
            ...
            print(f"[SRRHUIF] Ep {ep:3d} | Rwd: {ep_r:6.1f} | Avg20: {recent:6.1f} | eps: {eps:.2f} "
                  f"| Loss: {avg_l:.4f} | Z_var: {avg_v:.2f} | K_Gain: {avg_k:.4f} | Time: {time.time()-ep_start:.2f}s")
...
def main():
    print(f"\n{'#' * 70}")
    print(f"  SRRHUIF-ND ({'SPAS' if cfg.use_spas else 'Standard'}) Debugging Session")
